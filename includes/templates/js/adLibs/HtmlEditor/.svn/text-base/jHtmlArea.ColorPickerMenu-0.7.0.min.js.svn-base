			...
			if(c){
				...
				jHtmlAreaColorPickerMenu($(".forecolor",this.toolbar),{
					colorChosen:function(color){
						...
						$('iframe').contents().find('head').blur();
...
menu.fn=menu.prototype={
	...
	init:function(ownerElement,options){
		...
		var picker=this.picker=$("<div/>").css({
			...
			"z-index":300, "height":"200px;"
		}).addClass("jHtmlAreaColorPickerMenu");
		for(var i=0;i<opts.colors.length;i++){
			var c=opts.colors[i];
			$("<div/>").css("background-color",c).appendTo(picker).click((function(color){
				return function(){
					if(opts.colorChosen){
	...
	$("<div/>").html("<div style='display: none'></div>").addClass("automatic").appendTo(picker).click(function(){
		if(opts.colorChosen){
