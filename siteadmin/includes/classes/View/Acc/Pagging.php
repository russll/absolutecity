...
class Pagging
{
    ...
    public function &Make( &$gSmarty, $module = '', $func = '', $param = '' )
    {
    	$pages = '';
        ...
        if (1 < $this -> mPage)
        {
            $gSmarty -> assign('lprev', $func ? 'javascript:'.$func.'(\''.($this -> mPage - 1).'\', \''.$param.'\');' : $link.'page='.($this -> mPage - 1));
        }
        
        if ($this -> mPage < (ceil($this -> mEcount / $this -> mResOnPage)))
        {
            $gSmarty -> assign('lnext', $func ? 'javascript:'.$func.'(\''.($this -> mPage + 1).'\', \''.$param.'\');' : $link.'page='.($this -> mPage + 1));
            /*
        ...
        if($this -> mPage < $lastPage)
        	$gSmarty -> assign( 'oldest', $func ? 'javascript:'.$func.'(\''.$lastPage.'\', \''.$param.'\');' : $link.'page='.$lastPage );
        ...
        while ($k < $this -> mEcount)
        {
            ...
            if ($i >= $fl && $i <= $lr)
            {
                $res[] =  array('page' => $i, 'link' => $func ? 'javascript:'.$func.'(\''.$i.'\', \''.$param.'\');' : $link.'page='.$i);
            }
            $k     += $this -> mResOnPage;
            $i ++;
        }
